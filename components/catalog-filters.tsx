        ...
        {showAdvancedFilters && (
          ...
          <label className="catalog-select">
            ...
            <select name="year" defaultValue={values.year ?? ""}>
              ...
              {Array.from({ length: 10 }, (_, index) => currentYear - index).map((year) => <option value={year} key={year}>{year}</option>)}
            </select>
          </label>
          <label className="catalog-select">
            <span className="sr-only">Avaliação mínima</span>
            <select name="rating" defaultValue={values.rating ?? ""}>
              <option value="">Avaliação</option><option value="8">8 ou mais</option><option value="7">7 ou mais</option><option value="6">6 ou mais</option>
            </select>
          </label>
          <label className="catalog-select">
            <span className="sr-only">Idioma original</span>
            <select name="language" defaultValue={values.language ?? ""}>
              <option value="">Idioma</option><option value="pt">Português</option><option value="en">Inglês</option><option value="es">Espanhol</option><option value="ja">Japonês</option><option value="ko">Coreano</option>
            </select>
          </label>
          </>
        )}
