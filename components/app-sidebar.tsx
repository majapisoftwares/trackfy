...
export function AppSidebar() {
  ...
  return <aside className={`app-sidebar ${collapsed ? "is-collapsed" : ""}`} aria-label="Menu principal">
    ...
    <nav className="sidebar-nav">{mainItems.map(({ href, label, icon: Icon }) => <Link className={isActive(href) ? "active" : undefined} href={href} key={href}><Icon size={19} />{label}</Link>)}</nav>
    <nav className="sidebar-nav sidebar-nav-bottom"><Link className={isActive("/configuracoes") ? "active" : undefined} href="/configuracoes"><Settings size={19} />Configurações</Link><a href="mailto:suporte@trackfy.app"><CircleHelp size={19} />Ajuda</a></nav>
  </aside>;
}
